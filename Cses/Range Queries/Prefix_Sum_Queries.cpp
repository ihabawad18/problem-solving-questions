...
struct segmenttree {
  ...
  data query(int start,int ending,int left,int right,int node){
      if(ending < left || start>right){
        return data(0);
      }
      if (start >= left && ending <= right) {
         return st[node];
      }
      int mid=(start+ending)>>1;
      data q1=query(start,mid,left,right,node*2+1);
      data q2=query(mid+1,ending,left,right,node*2+2);
  ...
  data query(int l,int r){return query(0,n-1,l,r,0);}
...
void solve() {
    ...
    while(q--){
      ...
      if(type==1){
          cin >> l>>v;
          s.update(l-1,v);
          //cout<<s.query(l-1,l-1).sum<<endl;
      }
      else{
         cin >> l>>v;
         cout<<s.query(l-1,v-1).pref<<endl;
      }
  }
}
