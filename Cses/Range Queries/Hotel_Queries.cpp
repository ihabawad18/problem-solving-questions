...
struct segmenttree
{
    ...
    pair<int, int> query(int start, int ending, int x, int node)
    {
        if (st[node] < x)
            return {-1, -1};
        if (start == ending)
            return {start, st[node]};

        int mid = (start + ending) >> 1;
        auto left = query(start, mid, x, 2 * node + 1);
        if (left.first != -1)
            return left;
        return query(mid + 1, ending, x, 2 * node + 2);
    ...
    pair<int, int> query(int x) { return query(0, n - 1, x, 0); }
...
void solve()
{
    ...
    while (q--)
    {
        ...
        {
            cout << ans.first + 1 << " ";
            st.update(ans.first, ans.second - x);
        }
        else
        {
            cout << 0 << " ";
