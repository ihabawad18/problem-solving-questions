...
struct segmenttree
{
    ...
    int query(int start, int ending, int k, int node)
    {
        ...
        if (st[2 * node + 1] >= k)
        {
            return query(start, mid, k, 2 * node + 1);
        }
        return query(mid + 1, ending, k - st[2 * node + 1], 2 * node + 2);
    }
    ...
    int query(int k) { return query(0, n - 1, k, 0); }
...
void solve()
{
    int n;
    cin >> n;
    vi v(n), v1(n), temp(n, 1);
    cin >> v >> v1;
    segmenttree st;
    st.init(n, temp);
    for (int i = 0; i < n; i++)
    {
        int index = st.query(v1[i]);
        cout << v[index] << " ";
        st.update(index, 0);
    }
}
