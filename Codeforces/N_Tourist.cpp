...
struct segmenttree
{
    ...
    Item merge(Item a, Item b)
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            ans.prefix[i] = a.prefix[i] + b.prefix[i];
            ans.mins[i] = min(a.mins[i], b.mins[i]);
        }
        return ans;
    }
    ...
    {
        Item res = query(0, n - 1, l, r, 0);
        ll ans = 0;
        for (int i = 0; i < 32; i++)
        {
            if (res.mins[i] == LONG_MAX)
                continue;
            if (res.mins[i] > ans + 1)
                break;
            ans += res.prefix[i];
